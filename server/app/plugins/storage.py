            ...
            if is_enveloped:
                try:
                    ...
                    if not data_url.startswith("data:application/vc+jwt,"):
                        ...
                        raise ValueError(
                            f"EnvelopedVerifiableCredential must use 'application/vc+jwt' media type. "
                    ...
                    if len(parts) == 3:
                        ...
                        if "credentialSubject" not in decoded_vc:
                            raise ValueError("JWT payload in EnvelopedVerifiableCredential must have 'credentialSubject' field")
                        ...
                        raise ValueError("Invalid JWT format in EnvelopedVerifiableCredential - must have 3 parts (header.payload.signature)")
                except Exception as e:
